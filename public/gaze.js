function Gaze()
{
    ...
    this.cursor = function()
    {
        ...
        this.eye.position.x = this.x - this.offset;
        this.eye.position.y = this.y - this.offset;
    ...
    this.check = function()
    {
        if(bIsEye == true)
        {
            gaze.eye.changeAnimation("eyes");
        }
        if(bIsEye == false)
        {
            gaze.eye.changeAnimation("hammer");
        }
    }
    ...
    {
        var currentMillis = millis();
        if(currentMillis - previousMillisVamp > interval)
        {
            if(bIsEye == true)
            {
                if(heart.lives > 0)
